...
const oauth2Client = new google.auth.OAuth2(
  ...
  'urn:ietf:wg:oauth:2.0:oob' // For non-interactive flows
);
...
class ColabMcpServer {
  ...
  private async loadTasksFromDrive() {
    try {
  ...
  private async uploadFile(filename: string, content: string, folderId: string = DRIVE_FOLDER_ID): Promise<string> {
    const fileMetadata = {
      name: filename,
      parents: [folderId]
    };
  ...
  async uploadCode(code: string, filename: string, requirements: string[] = []): Promise<Task> {
  ...
  async runCalculation(taskId: string, executionParams: Record<string, any> = {}): Promise<Task> {
  ...
  async authenticate(testConnection: boolean = false): Promise<any> {
    try {
  ...
  async listTasks(statusFilter?: string, limit: number = 10): Promise<Task[]> {
